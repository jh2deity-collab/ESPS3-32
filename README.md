...
먼저 저장소를 받고 **그 폴더 안으로 들어가야** 한다.
아래 명령들은 모두 프로젝트 폴더(`ESPS3-32`) 기준이다.

```bash
git clone https://github.com/jh2deity-collab/ESPS3-32.git
cd ESPS3-32
```

git 이 없다면 GitHub 저장소 페이지에서 **Code → Download ZIP** 으로 받아
압축을 푼 뒤, 그 폴더로 이동하면 된다.

> 지금 어느 폴더에 있는지 헷갈리면 확인해 보자.
> 프로젝트 폴더가 맞다면 `web`, `firmware`, `tools` 가 보여야 한다.
>
> - 윈도우(PowerShell): `pwd` 그리고 `dir`
> - macOS/Linux: `pwd` 그리고 `ls`
>
> `C:\Users\사용자이름>` 같은 위치에서 실행하면
> *"경로를 찾을 수 없습니다" / "-File 매개 변수에 대한 인수 ... 이(가) 없습니다"*
> 오류가 난다. 파일이 없어서지 프로그램 문제가 아니다.
...
| `-File 매개 변수에 대한 인수 ...이(가) 없습니다` / `경로를 찾을 수 없습니다` | 프로젝트 폴더 밖에서 실행했다 | `cd` 로 `ESPS3-32` 폴더에 들어간 뒤 실행. `dir` 에 `web` `firmware` `tools` 가 보여야 한다 |
